function [] = turnLeft(pos_body,pos_head,pos_tail,vec,per,t_turn,t)
p=perception();
tail_vec=pos_body-pos_tail;
angle=acosd(dot(vec,tail_vec)/(norm(vec)*norm(tail_vec)));
if angle >= 119
    turnRight(pos_body,pos_head,pos_tail,vec,per,t_turn,t);
end
...
if 20>p_turn_terminate
    turnLeft(pos_body,pos_head,pos_tail,new_vec,per,t_turn,t);
end

...
check=0;
a=getGlobalx; %get arena
angle=acosd(dot(vec,vec_tail)/(norm(vec)*norm(vec_tail))); %calculate the angle between vec and tail_vec
if angle>119
    setGlobalz(3); %this will call recenter
    check=3;
end
...
p_turn_terminate=(turn_terminate_base+t_turn_kernel*T)*0.1;
t_turn=t_turn+1;
setGlobaly(per);
